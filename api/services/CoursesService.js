...
module.exports = {
  async createCourse(day, startDate, endDate, CourseName, classe, createdBy) {
    const id = uuidv4();
    const query = `
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX educa: <http://www.semanticweb.org/gueddes/ontologies/2015/e-Saad-Educa#>
        PREFIX course: <http://www.semanticweb.org/gueddes/ontologies/2015/e-Saad-Educa/course/>
        INSERT DATA {
            course:${id} rdf:type educa:course;
                educa:course_id "${id}";
                educa:day "${day}";
                educa:startDate "${startDate}";
                educa:endDate"${endDate}";
                educa:CourseName "${CourseName}";
                educa:classe "${classe}";
  ...
  async CoursesByClasse(classe) {
    const sparqlQuery = `
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX educa: <http://www.semanticweb.org/gueddes/ontologies/2015/e-Saad-Educa#>
        PREFIX course: <http://www.semanticweb.org/gueddes/ontologies/2015/e-Saad-Educa/course/>
        SELECT ?course ?day ?startDate ?endDate ?CourseName ?classe 
        WHERE {
            ?course educa:classe "${classe}";
                educa:day ?day;
                educa:startDate ?startDate;
                educa:endDate ?endDate;
                educa:CourseName ?CourseName;
